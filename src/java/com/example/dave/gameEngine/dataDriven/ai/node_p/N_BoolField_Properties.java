...
import com.example.dave.gameEngine.ai.decisionTree.N_BoolFieldPredicate;
...
public class N_BoolField_Properties extends N_Predicate_Properties {
	public Field_Properties<Boolean> field=null;
	...
	public DTNode_Properties clone() {
		N_BoolField_Properties newInstance = new N_BoolField_Properties();
	...
	public PropertyException getErrors() {
		if(!isReady()){
			String msg="N_BoolField_Properties not ready:";
	...
	public Node build(Object caller) {
		return new N_BoolFieldPredicate(onTrue.build(caller), onFalse.build(caller), field.build(caller));
	}
}
